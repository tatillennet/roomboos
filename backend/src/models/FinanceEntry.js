...
/* source canonicalization (route ile uyumlu) */
const SOURCE_CANON = {
...
const FinanceEntrySchema = new Schema({
  ...
  method:   { type: String, required: true, enum: ['cash','pos','transfer','online','other'], set: normMethod, default: 'cash', index: true },
  ...
  ref:  { type: String, default: '', trim: true }, // CSV/arama için
...
FinanceEntrySchema.index({ hotel: 1, method: 1, date: -1 });
...
// findOneAndUpdate senaryosunda amountTry yeniden hesapla
FinanceEntrySchema.pre('findOneAndUpdate', function(next) {
  const u = this.getUpdate() || {};
  const $set = u.$set || (this._update.$set = {});

  if (u.currency || $set.currency) {
    const cur = upper($set.currency ?? u.currency, 'TRY');
    $set.currency = cur;
    if (cur === 'TRY' && ($set.fxRate === undefined && u.fxRate === undefined)) {
      $set.fxRate = 1;
    }
  }
  if (u.method || $set.method) {
    $set.method = normMethod($set.method ?? u.method);
  }
  if (u.source || $set.source) {
    $set.source = normSource($set.source ?? u.source);
  }
