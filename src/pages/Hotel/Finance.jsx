...
// GELİRLER: otel ihtiyacına göre
const INCOME_CATS  = [
  'Kapora Geliri',
  'Rezervasyon Kalan Ödemesi',
  'Rezervasyon Ödemesi',
  'Ekstra Harcama','Restoran/Bar','Mini Bar','Spa','Diğer Gelir'
]

// GİDERLER: öne çıkan muhasebe kalemleri
const EXPENSE_CATS = [
  'Acente Komisyonu',
  'Finansal Komisyon',
  'Vergiler',
...
const ymKey = (d) => { const x=new Date(d); const y=x.getFullYear(); const m=String(x.getMonth()+1).padStart(2,'0'); return `${y}-${m}` }
const monthName = (ym) => new Date(`${ym}-01T00:00:00`).toLocaleDateString('tr-TR', {month:'long', year:'numeric'})
...
export default function Finance() {
  ...
  const monthlyPnl = useMemo(()=>{
    const keys = monthKeysBetween(filters.start, filters.end)
    const zero = () => ({
      kapora:0, kalan:0, rez:0, digerGelir:0,
      acente:0, finansal:0, vergiler:0, personel:0, fatura:0, digerGider:0
    })
    const map = new Map(keys.map(k=>[k, zero()]))

    items.forEach(r=>{
      const key = ymKey(r.date||r.createdAt||new Date())
      if (!map.has(key)) map.set(key, zero())
      const row = map.get(key)
      const tryAmt = Number(r.amountTry ?? (Number(r.amount||0)*Number(r.fxRate||1))) || 0
      if (r.type === 'income') {
        const c = r.category || ''
        if (c==='Kapora Geliri') row.kapora += tryAmt
        else if (c==='Rezervasyon Kalan Ödemesi') row.kalan += tryAmt
        else if (c==='Rezervasyon Ödemesi') row.rez += tryAmt
        else row.digerGelir += tryAmt
      } else {
        const c = r.category || ''
        if (c==='Acente Komisyonu') row.acente += tryAmt
        else if (c==='Finansal Komisyon') row.finansal += tryAmt
        else if (c==='Vergiler' || c==='Vergi/Harç') row.vergiler += tryAmt
        else if (c==='Personel Giderleri' || c==='Maaş/SGK') row.personel += tryAmt
        else if (c==='Fatura Giderleri') row.fatura += tryAmt
        else row.digerGider += tryAmt
      }
    })

    const rows = keys.map(k=>{
      const r = map.get(k) || zero()
      const gelirTop = r.kapora + r.kalan + r.rez + r.digerGelir
      const giderTop = r.acente + r.finansal + r.vergiler + r.personel + r.fatura + r.digerGider
      const net = gelirTop - giderTop
      return { k, ...r, gelirTop, giderTop, net }
    })
    return rows
  }, [items, filters.start, filters.end])
  ...
  const buildEntriesFromReservations = (reservations, opts) => {
    ...
    reservations.forEach(r => {
      ...
      // kapora var mı? (check-in öncesi pozitif ödeme)
      const hasDeposit = payments.some(p=>{
        const amt = Number(p.amount || p.total || 0) || 0
        const dt  = p.date || p.createdAt || checkIn || new Date()
        return amt>0 && checkIn && new Date(dt) < new Date(checkIn)
      })
      ...
      if (opts.includePayments) {
        payments.forEach(p => {
          ...
          // kategori belirleme
          let category = 'Rezervasyon Ödemesi'
          if (!isRefund && checkIn) {
            if (new Date(dt) < new Date(checkIn)) category = 'Kapora Geliri'
            else category = hasDeposit ? 'Rezervasyon Kalan Ödemesi' : 'Rezervasyon Ödemesi'
          }

          out.push({
            type: isRefund ? 'expense' : 'income',
            method: m,
            category: isRefund ? 'İade/İptal' : category,
            amount: Math.abs(amt),
            currency: cur,
            fxRate: fx,
            date: iso(dt),
      ...
      if (opts.includePlannedBalance) {
        ...
        if (balance > 0 && checkIn) {
          out.push({
            type: 'income',
            method: 'transfer',
            category: hasDeposit ? 'Rezervasyon Kalan Ödemesi' : 'Rezervasyon Ödemesi',
            amount: balance,
            currency: 'TRY',
            fxRate: 1,
            date: iso(checkIn),
  ...
  /* >>> yeni: bölümler için aç/kapa state'leri */
  const [filterOpen, setFilterOpen] = useLocal('fin.filterOpen', false)
  const [createOpen, setCreateOpen] = useLocal('fin.createOpen', false)
  const [accountOpen, setAccountOpen] = useLocal('fin.accountOpen', true)

  /* ====== UI ====== */
  return (
    <div>
      <Header title="Gelir & Gider" subtitle="Ay bazlı muhasebe • Kapora/Kalan/Ödeme sınıflandırma • Döviz • Tekrarlı • CSV/PDF • Rezervasyon entegrasyonu" />
      ...
      <div className="card" style={{display:'flex',gap:8,alignItems:'center',marginBottom:12,flexWrap:'wrap'}}>
        ...
        <button
          ...
          style={{background:'linear-gradient(135deg,#34d399 0%,#22d3ee 100%)', border:'none', color:'#0b1220', fontWeight:600}}
        >
          {createOpen ? 'Gelir/Gider Ekle (Gizle)' : 'Gelir/Gider Ekle'}
        </button>
        ...
        <button
          ...
          style={{background:'linear-gradient(135deg,#fbbf24 0%,#f472b6 100%)', border:'none', color:'#0b1220', fontWeight:700}}
        >
          {accountOpen ? 'Muhasebe Özeti (Gizle)' : 'Muhasebe Özeti (Ay Bazlı)'}
        </button>
      ...
      {/* filtreler + özet — BUTONLA AÇ/KAPA */}
      <div style={{overflow:'hidden', transition:'max-height 0.25s ease', maxHeight: filterOpen ? 2000 : 0, marginBottom: filterOpen ? 16 : 0}}>
        <div className="card">
      ...
      {/* ===== Ay Bazlı Muhasebe Özeti ===== */}
      <div style={{overflow:'hidden', transition:'max-height 0.25s ease', maxHeight: accountOpen ? 4000 : 0, marginBottom: accountOpen ? 16 : 0}}>
        <div className="card">
          <div className="label" style={{marginBottom:8}}>Muhasebe Özeti (Ay Bazlı)</div>
          <div style={{overflowX:'auto'}}>
            <table className="table">
              ...
              <tbody>
                {monthlyPnl.map(r=>(
                  <tr key={r.k}>
                    <td style={{fontWeight:600}}>{monthName(r.k)}</td>
                    <td>{formatTRY(r.kapora)}</td>
                    <td>{formatTRY(r.kalan)}</td>
                    <td>{formatTRY(r.rez)}</td>
                    <td>{formatTRY(r.digerGelir)}</td>
                    <td style={{fontWeight:700}}>{formatTRY(r.gelirTop)}</td>

                    <td>{formatTRY(r.acente)}</td>
                    <td>{formatTRY(r.finansal)}</td>
                    <td>{formatTRY(r.vergiler)}</td>
                    <td>{formatTRY(r.personel)}</td>
                    <td>{formatTRY(r.fatura)}</td>
                    <td>{formatTRY(r.digerGider)}</td>
                    <td style={{fontWeight:700}}>{formatTRY(r.giderTop)}</td>

                    <td style={{fontWeight:800, color: r.net>=0 ? '#10b981' : '#ef4444'}}>{formatTRY(r.net)}</td>
                  </tr>
                ))}
                {monthlyPnl.length===0 && <tr><td colSpan={14} className="muted">Aralıkta veri yok</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {/* yeni kayıt formu — BUTONLA AÇ/KAPA */}
      <div style={{overflow:'hidden', transition:'max-height 0.25s ease', maxHeight: createOpen ? 2000 : 0, marginBottom: createOpen ? 16 : 0}}>
        <div className="card">
      ...
      <Drawer open={syncOpen} onClose={()=>setSyncOpen(false)} title="Rezervasyonlardan Gelir/Gider Aktar">
        <div className="muted" style={{marginBottom:8}}>
          Seçilen tarih aralığındaki rezervasyonlardan ödemeleri, <b>check-in öncesini “Kapora”</b>,
          check-in günü/sonrasını <b>“Rezervasyon Ödemesi/Kalan Ödeme”</b> olarak aktarır.
