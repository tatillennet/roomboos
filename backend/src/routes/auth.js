...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!process.env.JWT_SECRET) {
      const e = new Error('JWT_SECRET missing'); e.status = 500; throw e
    }

    // password alanı select:false, bu yüzden +password ile seçiyoruz
    const user = await User.findOne({ email })
      .select('+password')
      .populate('hotel', 'name code')

    // kullanıcı yok veya şifre yanlış -> aynı mesaj (enumeration engeli)
    if (!user) return res.status(401).json({ message: 'Email veya şifre hatalı' })

    // hem comparePassword hem compare için uyumluluk
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(userId).populate('hotel', 'name code')
