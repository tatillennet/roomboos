...
/* FinanceEntry source enum: manual | res_payment | res_balance | res_refund | system */
const SOURCE_CANON = {
...
/** Hotel scope: non-master → token hotel; master → ?hotelId varsa onu; yoksa tümü */
function scopeHotelFilter(req) {
...
function hotelForCreate(req, body) {
  const role = req.user?.role;
  const tokenHotel = req.user?.hotel?._id || req.user?.hotel;
  if (role !== 'MASTER_ADMIN') return tokenHotel || null;
  const fromBody = body?.hotel;
  const fromQuery = req.query?.hotelId;
  const chosen = fromBody || fromQuery || tokenHotel;
  return isObjId(chosen) ? chosen : null;
}
...
router.get(
  ...
  validate([
    ...
    query('hotelId').optional().custom((v) => isObjId(v) || v === ''),
  ]),
  ah(async (req, res) => {
    ...
    if (req.query.reservation) filter.reservation = new mongoose.Types.ObjectId(req.query.reservation);
    if (start || end) {
      filter.date = {};
    ...
    // amountTry saklı olmayan eski kayıtlar için amount*fxRate hesapla
    const amountTryExpr = {
      $cond: [
        { $gt: [{ $ifNull: ['$amountTry', 0] }, 0] },
        '$amountTry',
...
router.get(
  ...
  ah(async (req, res) => {
    const filter = { ...scopeHotelFilter(req), _id: req.params.id };
    const item = await FinanceEntry.findOne(filter);
...
router.post(
  ...
  ah(async (req, res) => {
    const hotel = hotelForCreate(req, req.body);
...
router.post(
  ...
  ah(async (req, res) => {
    const hotel = hotelForCreate(req, {});
    if (!hotel) return res.status(400).json({ message: 'Hotel context yok' });
...
router.put(
  ...
  ah(async (req, res) => {
    ...
    const update = {};
    const fields = ['type','category','note','ref','reservation','guestName','channel','source'];
    fields.forEach((k) => (k in b ? (update[k] = k === 'source' ? normSource(b[k]) : b[k]) : null));
    ...
    const updated = await FinanceEntry.findOneAndUpdate(filter, update, { new: true });
...
router.delete(
  ...
  ah(async (req, res) => {
    const filter = { ...scopeHotelFilter(req), _id: req.params.id };
    const del = await FinanceEntry.findOneAndDelete(filter);
...
router.get(
  ...
  auth,
  validate([
    query('groupBy').optional().isIn(['category', 'method', 'source', 'reservation', 'channel']),
    query('start').optional().isISO8601(),
    query('end').optional().isISO8601(),
    query('type').optional().isIn(['income', 'expense']),
    query('reservation').optional().isMongoId(),
  ...
  ah(async (req, res) => {
    ...
    const match = { ...scopeHotelFilter(req) };
    if (req.query.type) match.type = req.query.type;
    if (req.query.reservation) match.reservation = new mongoose.Types.ObjectId(req.query.reservation);
    if (start || end) {
      match.date = {};
...
router.get(
  ...
  ah(async (req, res) => {
    ...
    const rows = [
      ['Tarih','Tip','Yöntem','Kategori','Tutar','Para','Kur','Tutar(TRY)','Not','Ref','Misafir','Kanal','Rezervasyon'],
      ...items.map((e) => [
