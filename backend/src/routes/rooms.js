...
function hotelFrom(req) {
  // MASTER isteklerinde ?hotelId gelirse onu kullan; yoksa tüm oteller ({} filtresi)
  if (req.user.role === 'MASTER_ADMIN') return req.query.hotelId || req.body.hotelId || null;
  return req.user.hotel?._id || req.user.hotel;
}
...
router.get(
  '/types',
  auth,
  validate([ query('hotelId').optional().custom((v) => isObjId(v) || v === '') ]),
  async (req, res) => {
    const hotel = hotelFrom(req);
    const filter = hotel ? { hotel } : {}; // master + hotelsiz istek → tümü
    const items = await RoomType.find(filter).sort({ name: 1 }).lean();
    res.json(items);
  }
);
...
router.post(
  ...
  validate([
    ...
    body('hotelId').optional().custom((v) => isObjId(v) || v === ''),
  ]),
  async (req, res) => {
    const tokenHotel = req.user.hotel?._id || req.user.hotel;
    const hotel = (req.user.role === 'MASTER_ADMIN' ? (req.body.hotelId || tokenHotel) : tokenHotel);
...
/* =========================
   ...
   (aktif rezervasyon varsa engelle)
========================= */
router.delete(
  '/types/:id',
  auth, requireRole('HOTEL_ADMIN'),
  ...
  async (req, res) => {
    ...
    const hasRes = await Reservation.exists({
      hotel,
      roomType: req.params.id,
      status: { $ne: 'cancelled' }
    });
    if (hasRes) {
      return res.status(400).json({ message: 'Bu oda tipine bağlı aktif rezervasyonlar var. Silemezsiniz.' });
    }
...
/* =========================
   INVENTORY — GET
   GET /api/rooms/inventory?roomType&start&end[&hotelId]
========================= */
router.get(
  '/inventory',
  auth,
...
router.post(
  ...
  async (req, res) => {
    ...
    const toSet = {};
    if (price !== undefined) toSet.price = Number(price);
    if (allotment !== undefined) toSet.allotment = Number(allotment);
...
/* =========================
   AVAILABILITY QUOTE
   GET /api/rooms/availability/quote?roomType&start&end&rooms=1[&hotelId]
   → { nights, remainingPerDay:[{date, allotment, used, remaining, price, stopSell}], available, suggestedTotalPrice }
========================= */
router.get(
  '/availability/quote',
  auth,
  ...
  async (req, res) => {
    ...
    const days = [];
    for (let t = s.getTime(); t < e.getTime(); t += DAY) days.push(new Date(t));

    const invList = await Inventory.find({ hotel, roomType, date: { $gte: s, $lt: e } }).lean();

    // Çakışan confirmed rezervasyonların gün bazında kullanılan oda adetleri
    const usedAgg = await Reservation.aggregate([
      { $match: { hotel: new mongoose.Types.ObjectId(hotel), roomType: new mongoose.Types.ObjectId(roomType), status: 'confirmed', checkIn: { $lt: e }, checkOut: { $gt: s } } },
